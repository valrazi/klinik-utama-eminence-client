...
export default function DataGuruEditPage() {
    ...
    const fetchDetail = async () => {
        try {
            let data = await axios.get(`${API_URL}/expense/` + id)
            data = data.data.data
            if (data) {
    ...
    const onSubmit = async (data) => {
        ...
        for (const key in data) {
            ...
            } else if(key == 'nominal_pengeluaran') {
        ...
        try {
            setLoading(true)
            await axios.put(`${API_URL}/expense/` + id, formData)
    ...
    return (
        <Container title={'Tambah Pengeluaran'}>
            <Flex justify={'space-between'} align={'center'} pb={'2'} px={'5'}
                ...
                <Flex gapX={'4'} justify={'end'} w='full'>
                    <NavigationButton
                        onClick={() => {
