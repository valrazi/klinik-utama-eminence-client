...
export default function DataGuruEditPage() {
    ...
    const fetchDetail = async () => {
        try {
            let data = await axios.get(`${API_URL}/schedule-lesson/` + id)
            data = data.data.data
            if (data) {
            ...
            alert('Fetch schedule lesson failed')
        }
    }
    ...
    const onSubmit = async (data) => {
        ...
        try {
            setLoading(true)
            await axios.put(`${API_URL}/schedule-lesson/` + id, formData)
    ...
    return (
        <Container title={'Edit Jadwal Pelajaran'}>
            ...
            <Flex px={'5'} direction={'column'}>
                    <Heading width={'fit'} fontWeight={'bold'} borderBottom={'4px solid yellow'} marginBottom={'30px'}>Data  Jadwal Pelajaran</Heading>
