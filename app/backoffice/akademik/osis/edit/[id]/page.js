...
export default function DataGuruEditPage() {
    ...
    const fetchDetail = async () => {
        try {
            let data = await axios.get(`${API_URL}/osis/` + id)
            data = data.data.data
            if (data) {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            setLoading(true)
            await axios.put(`${API_URL}/osis/` + id, formData)
    ...
    return (
        <Container title={'Edit OSIS Sekolah'}>
            <Flex justify={'space-between'} align={'center'} pb={'2'} px={'5'}
                ...
                <Flex gapX={'4'}>
                    <NavigationButton
                        onClick={() => {
                            router.push('/backoffice/akademik/osis')
                        }}
                        label="Kembali" />
                </Flex>
            </Flex>
            ...
            <Flex px={'5'} direction={'column'}>
                    <Heading width={'fit'} fontWeight={'bold'} borderBottom={'4px solid yellow'} marginBottom={'30px'}>Data OSIS</Heading>
                
                <form onSubmit={handleSubmit(onSubmit)}>
                    <VStack spacing={4} align="stretch">
                        <HStack gapX={'2rem'}>
                            <VStack width={'1/2'} justify={'start'} align={'start'}>
                               <FormControl isInvalid={errors.penjelasan_osis} style={{ width: '100%' }}>
                                    <FormLabel fontWeight={'bold'}>Penjelasan OSIS</FormLabel>
                                    <Textarea
                                        rows={'4'}
                                        border="1px solid gray"
                                        {...register('penjelasan_osis', { required: 'Penjelasan OSIS wajib diisi' })}
                                    />
                                    <FormErrorMessage fontSize={'12px'} color={'red'}>{errors.penjelasan_osis?.message}</FormErrorMessage>
                                </FormControl>
                            ...
                            <FormControl isInvalid={errors.foto} p={'4em'} style={{ borderRadius: '1rem' }} border={'1px solid black'}>
                                ...
                                <FormLabel>Foto Struktur OSIS</FormLabel>
                                <Input
                                    type="file"
                                    accept="image/*"
                                    {...register('foto', {
