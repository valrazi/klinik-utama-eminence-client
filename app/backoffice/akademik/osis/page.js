...
export default function BackofficePage() {
    ...
    const fetchUsers = async (name = null) => {
        try {
            const data = await axios.get(`${API_URL}/osis`)
    ...
    const addData = () => {
        router.push('/backoffice/akademik/osis/add')
    }
    const headers = ["No", "Penjelasan Osis", "Gambar Struktur", "Actions"];

    const renderRow = (item, idx) => (
        <Table.Row key={item.id} style={{ borderBottom: '1px solid black' }}>
            <Table.Cell textAlign="center">{idx + 1}</Table.Cell>

            <Table.Cell textAlign="center">{item.penjelasan_osis}</Table.Cell>
            <Table.Cell textAlign="center" display={'flex'} align="center" justifyContent={'center'}>
                <Image src={item.foto} alt="Gambar Struktur OSIS" width={'128'} height={'128'}/>
            </Table.Cell>
            <Table.Cell textAlign="center">
                <Flex justify="center" gap="4" justifyContent={'center'}>
                    <NavigationButton
                        onClick={() => {
                            router.push('/backoffice/akademik/osis/edit/' + item.id)
                        }}
