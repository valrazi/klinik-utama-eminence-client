...
export default function BackofficePage() {
    ...
    const fetchUsers = async (name = null) => {
        try {
            const data = await axios.get(`${API_URL}/scholarship`, {
                params: {
                    nama_lengkap: name
                }
            })
    ...
    const deleteUser = async (id) => {
        try {
            ...
            if(isConfirmed) {
                await axios.delete(`${API_URL}/scholarship/${id}`)
    ...
    const addData = () => {
        router.push('/backoffice/administrasi/permohonan-beasiswa/add')
    }
        ...
        <Table.Row key={item.id} style={{borderBottom: '1px solid black'}}>
            ...
            <Table.Cell textAlign="center">
                <Flex justify="center" gap="4" justifyContent={'center'}>
                    <NavigationButton
                    onClick={() => {
                        router.push('/backoffice/administrasi/permohonan-beasiswa/edit/' + item.id)
                    }}
